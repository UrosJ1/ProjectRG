...
vec3 moon(){
    ...
    vec3 reflectDirectionMoon = reflect(-directionOfLightMoon, normMoon);
    float specMoon = pow(max(dot(viewDirectionMoon, reflectDirectionMoon), 0.0f), 32.0f);
    vec3 specularMoon = lightColorMoon * specMoon;
    if (diffMoon <= 0.0f){
        specMoon = 0.0f;
    }

    return ambientMoon + diffuseMoon + specularMoon;
}
void main(){
    vec3 sunLight = sun();
    vec3 moonLight = moon();

    vec3 combined = sunLight + moonLight;
